app.controller('materiCtrl', function($scope, Data, toaster) {
    var tableStateRef;
    var control_link = "m_materi";
    $scope.formTitle = '';
    $scope.displayed = [];
    ...
    /** get roles list */
    $scope.getHari = function() {
        Data.get('m_hari/index').then(function(data) {
            $scope.listHari = data.data.list;
        });
    }

    $scope.list = function(id) {
        Data.get('m_materi/listmateri/' + id).then(function(data) {
            $scope.listmateri = data.data.data;
        });
    }
    /** create */
